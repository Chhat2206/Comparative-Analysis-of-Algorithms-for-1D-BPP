...
public class BinPackingGA {
    ...
    private static Individual crossover(Individual parent1, Individual parent2, int binCapacity) {
        ...
        // Merge items from both parents to offspring, avoiding duplicates
        ...
        // Identify remaining items not included in offspring
        ...
        // Reintegrate using a dynamic heuristic based on bin fullness
        reintegrateItemsDynamically(offspringBins, allItems, binCapacity);
    ...
    private static void reintegrateItemsDynamically(List<Bin> bins, Set<Item> remainingItems, int binCapacity) {
        ...
        boolean useBestFit = shouldUseBestFit(bins, binCapacity);

        for (Item item : sortedItems) {
            if (useBestFit) {
                // Integrate using Best-Fit
                integrateUsingBestFit(bins, item, binCapacity);
            } else {
                // Integrate using First-Fit or another heuristic
                integrateUsingFirstFit(bins, item, binCapacity);
            }
        }
    }
